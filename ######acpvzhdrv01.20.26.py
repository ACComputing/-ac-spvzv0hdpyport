...
# New menu colors (PVZ1 style)
MENU_SKY = (135, 206, 250)         # light sky blue
MENU_LAWN = (80, 140, 80)          # grass green
MENU_GRID = (50, 100, 50)          # darker green for grid
MENU_BUTTON = (230, 180, 100)      # seed packet beige
MENU_BUTTON_BORDER = (160, 100, 40)
MENU_BUTTON_SHADOW = (60, 40, 20)
MENU_BUTTON_SELECTED = (255, 220, 150)
MENU_TITLE = (255, 255, 100)       # bright yellow
...
class Game:
    def __init__(self):
        pygame.init()
        pygame.display.set_caption("AC'S PVZ Engine v0")
        ...
        for event in pygame.event.get():
            ...
            if self.state == "main_menu":
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    mx, my = event.pos
                    # Approximate button positions (same as drawn)
                    btn_width = 300
                    btn_height = 70
                    y_start = 250
                    for i in range(4):
    ...
    def draw_main_menu(self):
        # Sky (top 40%)
        sky_height = int(SCREEN_HEIGHT * 0.4)
        pygame.draw.rect(self.screen, MENU_SKY, (0, 0, SCREEN_WIDTH, sky_height))

        # Lawn (bottom 60%)
        lawn_top = sky_height
        lawn_height = SCREEN_HEIGHT - sky_height
        pygame.draw.rect(self.screen, MENU_LAWN, (0, lawn_top, SCREEN_WIDTH, lawn_height))

        # Draw lawn grid (using tile sizes)
        grid_color = MENU_GRID
        # Horizontal lines
        for r in range(lawn_height // TILE_H + 1):
            y = lawn_top + r * TILE_H
            pygame.draw.line(self.screen, grid_color, (0, y), (SCREEN_WIDTH, y), 1)
        # Vertical lines
        for c in range(SCREEN_WIDTH // TILE_W + 1):
            x = c * TILE_W
            pygame.draw.line(self.screen, grid_color, (x, lawn_top), (x, SCREEN_HEIGHT), 1)

        # Draw a sun in the sky
        pygame.draw.circle(self.screen, C_SUN, (150, 80), 40)
        pygame.draw.circle(self.screen, (255, 245, 160), (150, 80), 40, 3)

        # Title with shadow (PVZ1 style)
        title = "AC'S PVZ Engine v0"
        shadow = self.font_large.render(title, True, (0, 0, 0))
        text = self.font_large.render(title, True, MENU_TITLE)
        shadow_rect = shadow.get_rect(center=(SCREEN_WIDTH//2 + 6, 126))
        text_rect = text.get_rect(center=(SCREEN_WIDTH//2, 120))
        self.screen.blit(shadow, shadow_rect)
        self.screen.blit(text, text_rect)

        # Draw seed packet buttons
        ...
        for i, txt in enumerate(items):
            ...
            pygame.draw.rect(self.screen, MENU_BUTTON_SHADOW, shadow_rect, border_radius=12)
            ...
            # Small icon (simple plant-like shape)
            icon_rect = pygame.Rect(x + 15, y + 15, 40, 40)
            if i == 0:  # Adventure: peashooter
                pygame.draw.rect(self.screen, (70, 200, 80), icon_rect, border_radius=5)
                pygame.draw.circle(self.screen, (40, 120, 40), (x + 35, y + 25), 8)
            elif i == 1:  # Mini-games: sunflower
                pygame.draw.circle(self.screen, (240, 200, 40), (x + 35, y + 35), 15)
                pygame.draw.circle(self.screen, (60, 40, 10), (x + 30, y + 32), 3)
                pygame.draw.circle(self.screen, (60, 40, 10), (x + 40, y + 32), 3)
            elif i == 2:  # Almanac: book
                pygame.draw.rect(self.screen, (150, 100, 50), icon_rect)
                pygame.draw.line(self.screen, (100, 70, 30), (x + 25, y + 15), (x + 25, y + 55), 2)
            elif i == 3:  # Quit: door
                pygame.draw.rect(self.screen, (150, 80, 80), icon_rect)
                pygame.draw.circle(self.screen, (200, 200, 0), (x + 35, y + 35), 5)

            # Text (with black shadow and white text)
            draw_text(self.screen, txt, self.font_medium, (0, 0, 0), rect.centerx + 2, rect.centery + 2)
            draw_text(self.screen, txt, self.font_medium, (255, 255, 255), rect.centerx, rect.centery)
